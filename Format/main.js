...
function DataProcess() {
  ...
  //獲取“報到”的最新一筆資料
  var sheet = ss.getSheetByName("報到"); 
  ...
  var date = getDate(rawData[0]); //取得時間
  var name = getName(rawData[1]); //取得人名
  var summary = new Array(date,name); // 回傳
  return summary; 
  
}

function getName(rawname) {
  var split = rawname.slice(9,rawname.length);//資料格式：2019C002_孫仲雄，所以從第九個位置分離
  return split;
}
function getDate(rawdate) {
  var mm=rawdate.getMonth()+1;//0=January, 1=February 
  var dd=rawdate.getDate();
  return (mm.toString()+"/"+dd.toString());//傳回格式：月份/日期
}
