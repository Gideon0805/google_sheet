...
function checkRemittance() {
  ...
  const rawData = sheet.getRange(2, 2, lastRow-1, lastCol).getValues();//(start row, start col, row num, col num)//二維矩陣 取[0] 轉一維
  //2019/6/10 下午 8:54:29
  //var date = getDate("2019/6/10 下午 8:54:29");
  
  // 整理繳費名單
  // var remittResult = rawData.filter(isRemittance);
  var tempRow = sheet.getRange(1, 1, 1, lastCol).getValues()[0];
  ...
  const dataStart = 1;
  // 清空保險分頁資料
  insurance.clearContents();
  // 欄位名稱
  var colTitle = sheet.getRange(1, 2, 1, lastCol).getValues();
