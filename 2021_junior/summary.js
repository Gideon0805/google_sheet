...
function checkSummary() {
  ...
  const titleName = '全員逆轉—學院之戰(國中)人數統計（不含桂冠）';

  const rawData = nameSheet.getRange(startRow, 1, lastRow-1, lastCol-1).getValues();
  // titleRow 來找陣列的index;
  const titleRow = nameSheet.getRange(startRow-1, 1, 1, lastCol).getValues()[0];
  const gradeIndex = titleRow.indexOf('年級');
  const genderIndex = titleRow.indexOf('性別');
  const remitIndex = titleRow.indexOf('已繳費');
  const gradeCol = nameSheet.getRange(startRow, gradeIndex+1, lastRow-1, 1).getValues();
  const remitCol = nameSheet.getRange(startRow, remitIndex+1, lastRow-1, 1).getValues();
  const set = new Set();
  var tempContent;
  var content = [];
  tempContent = [titleName,'','',''];
